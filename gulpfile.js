...
var target = require('yargs').argv.target || '';
...
gulp.task('lint:html', function() {
  return gulp.src([
    ...
    path.join(target, 'src/**/*.html'),
    path.join(target, 'demo/**/*.html'),
    path.join(target, 'test/**/*.html')
  ])
    .pipe(htmlExtract({
...
gulp.task('version:check', function() {
  const expectedVersion = new RegExp('^' + require(path.join(target, './package.json')).version + '$');
  return gulp.src([path.join(target, 'src/*.html')])
...
gulp.task('version:update', ['version:check'], function() {
  ...
  const oldversion = require(path.join(target, './package.json')).version;
  ...
  return gulp.src([path.join(target, 'src/*.html')])
    ...
    .pipe(gulp.dest(path.join(target, 'src/.')))
    .pipe(git.add({cwd: target}));
});
